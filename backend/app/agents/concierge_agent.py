...
CONCIERGE_INSTRUCTIONS = (
    ...
    "data records. "
    "\n\nImportant: if the user's question mentions a graph, a view, what's "
    "'connected', what's 'here'/'on screen'/'this', or otherwise implies "
    "they're looking at something right now (e.g. 'why is this connected "
    "to X', 'summarize this', 'which of these is the parent', 'what does "
    "the current graph show') — ALWAYS hand off to master_data for that, "
    "even if you don't think you have a way to answer it yourself. Do not "
    "conclude on your own that you lack graph/screen access and answer "
    "directly — that determination belongs to master_data, which has a "
    "dedicated way to see exactly what's currently displayed. Only tell "
    "the user something isn't available after master_data itself says so. "
    "(note: as of now, "
    "searching, comparing, and Party create/update/delete requests are "
    "implemented underneath it — Item and Location requests are not yet, "
    ...
    "something goes beyond what's available). Data Catalog, Data "
    "Governance, and Data "
    ...
    "Solution'), not 'it'. "
    "\n\nThe master_data tool's reply may contain markdown links in the "
    "form [Name](record:ID) for specific records — that's how a record "
    "becomes clickable for the person you're replying to. Preserve that "
    "exact [Name](record:ID) formatting in your own final answer for any "
    "record you mention, whether passing one through unchanged or "
    "restating a record you already know the id of from earlier in the "
    "conversation — never flatten it to plain text like 'Name (ID: 123)'."
)
...
master_data_tool = agent_as_tool(
    ...
    description=(
        ...
        "or Location master data records — or about a relationship graph "
        "currently displayed on the user's screen (what's connected to "
        "what, and why)."
    ),
)
